...
module.exports = {
	...
	description: "Trộn nhạc", //the command description for Slash Command Overview
	cooldown: 2,
	...
	run: async (client, message, args) => {
		try {
			...
			let newMsg = await message.reply({
				content: `${client.allEmojis.loading} Loading **'${args[0] ? args[0] : "Default"}' Music Mix**`,
			});
			try {
				...
				//Edit the reply
				newMsg.edit({
					content: `${queue?.songs?.length > 0 ? "👍 Đã Load" : "🎶 Đang phát"}: the **'${args[0] ? args[0] : "Default"}'**`,
				});
			} catch (e) {
